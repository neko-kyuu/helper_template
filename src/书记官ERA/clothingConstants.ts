...
function generateRandomClothing(options: GenerateOptions = {}): GeneratedClothing {
    ...
    // 随机或使用指定值
    const tier = options.tier && validTiers.includes(options.tier) 
        ? options.tier 
        : randomChoice(validTiers);
    
    const quality = options.quality && validQualities.includes(options.quality)
        ? options.quality
        : randomChoice(validQualities);
    
    const slot = options.slot && validSlots.includes(options.slot)
        ? options.slot
        : randomChoice(validSlots);
